...
def init_speculator(vllm_config: VllmConfig, device: torch.device):
    ...
    elif speculative_config.method in ("ngram", "ngram_gpu"):
        from vllm.v1.worker.gpu.spec_decode.ngram.speculator import NgramSpeculator

        return NgramSpeculator(vllm_config, device)
    else:
